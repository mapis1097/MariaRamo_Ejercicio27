...
float implicita (float ini, float w, float delT);
...
for (int t =0; t< 4/omega; t+=deltaT){
        integral_implicita = implicita (inicial,omega,deltaT);
        inicial = integral_implicita;
        cout<<integral_implicita<<endl;
    }
return 0;
}
...
//metodo que retorna la integral implicita
float implicita (float ini, float w, float delT){
    return (ini/(1+ ( delT*w)));
}
